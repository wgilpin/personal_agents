...
async def load_flowchart_from_yaml(filepath: str) -> Optional[Dict]:
    """
...
async def build_custom_workflow_from_flowchart(flowchart_data: Dict) -> StateGraph:
    """
    ...
    for node in flowchart_data.get('nodes', []):
        ...
        if node_type == 'act':
            ...
            custom_workflow.add_node(node_id, execute_step)
        elif node_type == 'choice':
            # Choice nodes make a decision
            custom_workflow.add_node(node_id, assess_goal)
...
async def main():
    ...
    flowchart_data = await load_flowchart_from_yaml(flowchart_path)
    
    # Determine which workflow to use
    workflow_to_use = app  # Default workflow
    
    if flowchart_data:
        print(f"Using flowchart from {flowchart_path}")
        try:
            # Build a custom workflow from the flowchart
            custom_workflow = await build_custom_workflow_from_flowchart(flowchart_data)
